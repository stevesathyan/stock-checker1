...
# Initialize migration upgrades on launch
upgrade_database()
init_supplier_table()
...
def dashboard():
    ...
    total_parts = cursor.fetchone()[0]  # FIXED: Restored row indexes select keys
...
def update_part(part_id):
    ...
    cursor.execute("""
        UPDATE parts
        SET part_number = ?, brand = ?, stock = ?, purchase_price = ?, selling_price = ?, vehicle_model = ?
        WHERE id = ?
    """, (
        data.get("part_number"), data.get("brand"), data.get("stock"),
        data.get("purchase_price"), data.get("selling_price"),
        data.get("vehicle_model", "General"), part_id
    ))
    conn.commit()
    conn.close()
...
if __name__ == "__main__":
    print("APP STARTED — STABLE ACTIVE ENGINE")
    app.run(debug=True)